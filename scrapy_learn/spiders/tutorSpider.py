...
class TutorSpider(scrapy.Spider):
    ...
    def parse(self,response):
        for tutor in response.xpath("//div[@class='l fl']"):
            href = tutor.xpath("a/@href").extract_first()
...
# 编写爬取方法
    def parse_details(self, response):
        ...
        item['name'] = response.xpath("//div[@class='r fr']/h3/text()").extract_first()
        item['position'] = response.xpath("//div[@class='r fr']/p/text()").extract_first()
